...
#define UART_NUMBER_DATA_BYTES_TO_RECEIVE   4   // change this value depending on how many data bytes there is to receive ( Package = one start byte + data bytes + one byte 8 bit CRC )
#define UART_NUMBER_DATA_BYTES_TO_SEND      6   // change this value depending on how many data bytes there is to send ( Package = one start byte + data bytes + one byte 8 bit CRC )
#else
...
volatile uint8_t ui8_street_mode = 0;
volatile uint8_t ui8_target_battery_max_power_enabled = 1;
...
static void init_default_config(void)
{
  ...
  // assist level
  m_configuration_variables.ui8_assist_level_factor_x10 = ui8_assist_levels_factor_x10[ui8_assist_level];

  // lights state
  m_configuration_variables.ui8_lights = DEFAULT_LIGHTS_STATE;
  ...
  // street mode
  ui8_street_mode = DEFAULT_STREET_MODE_FUNCTION_ENABLED && DEFAULT_STREET_MODE_ENABLED_ON_STARTUP;

  // battery max power target
  m_configuration_variables.ui8_target_battery_max_power_div25 = (uint8_t) (ui8_target_battery_max_power_enabled ? ((uint16_t) DEFAULT_TARGET_BATTERY_MAX_POWER / 25) : 0);

#if DEFAULT_STREET_MODE_POWER_LIMIT_ENABLED
  if (ui8_street_mode)
    m_configuration_variables.ui8_target_battery_max_power_div25 = (uint8_t) ((uint16_t) DEFAULT_STREET_MODE_POWER_LIMIT / 25);
#endif

  // battery low voltage cut-off
  m_configuration_variables.ui16_battery_low_voltage_cut_off_x10 = DEFAULT_BATTERY_LOW_VOLTAGE_CUT_OFF_X10;
  ...
  // wheel perimeter
  m_configuration_variables.ui16_wheel_perimeter = DEFAULT_WHEEL_PERIMETER;

  // wheel max speed
  m_configuration_variables.ui8_wheel_max_speed = ui8_street_mode ? DEFAULT_STREET_MODE_SPEED_LIMIT : DEFAULT_WHEEL_MAX_SPEED;

  // battery max current
  m_configuration_variables.ui8_battery_max_current = DEFAULT_BATTERY_MAX_CURRENT;
  ...
  m_configuration_variables.ui8_motor_type = DEFAULT_MOTOR_TYPE;

  // startup motor power boost state
  m_configuration_variables.ui8_startup_motor_power_boost_state = DEFAULT_STARTUP_MOTOR_POWER_BOOST_STATE;

  // startup power boost max power limit enabled
  m_configuration_variables.ui8_startup_motor_power_boost_limit_to_max_power = DEFAULT_STARTUP_MOTOR_POWER_BOOST_LIMIT_TO_MAX_POWER;

  // startup motor power boost
  m_configuration_variables.ui8_startup_motor_power_boost_assist_level = ui8_startup_motor_power_boost_assist_levels[ui8_assist_level];

  // startup motor power boost time
  m_configuration_variables.ui8_startup_motor_power_boost_time = DEFAULT_STARTUP_MOTOR_POWER_BOOST_TIME_X10;

  // startup motor power boost fade time
  m_configuration_variables.ui8_startup_motor_power_boost_fade_time = DEFAULT_STARTUP_MOTOR_POWER_BOOST_FADE_TIME_X10;

  // startup motor boost enabled
  m_configuration_variables.ui8_startup_motor_power_boost_feature_enabled = DEFAULT_STARTUP_MOTOR_POWER_BOOST_FEATURE_ENABLED;

  // motor over temperature min value limit
  m_configuration_variables.ui8_motor_temperature_min_value_to_limit = DEFAULT_MOTOR_TEMPERATURE_MIN_VALUE_TO_LIMIT;

  // motor over temperature max value limit
  m_configuration_variables.ui8_motor_temperature_max_value_to_limit = DEFAULT_MOTOR_TEMPERATURE_MAX_VALUE_TO_LIMIT;

  // ramp up, amps per second
  m_configuration_variables.ui8_ramp_up_amps_per_second_x10 = DEFAULT_RAMP_UP_AMPS_PER_SECOND_X10;
  ...
  m_configuration_variables.ui8_temperature_limit_feature_enabled = DEFAULT_TEMPERATURE_LIMIT_FEATURE_ENABLED;

  // street mode throttle enabled
  if ((ui8_street_mode) && (m_configuration_variables.ui8_temperature_limit_feature_enabled == TEMPERATURE_LIMIT_FEATURE_THROTTLE))
    m_configuration_variables.ui8_temperature_limit_feature_enabled = DEFAULT_STREET_MODE_THROTTLE_ENABLED ? TEMPERATURE_LIMIT_FEATURE_THROTTLE : TEMPERATURE_LIMIT_FEATURE_NONE;

  // motor assistance without pedal rotation enable/disable when startup 
  m_configuration_variables.ui8_motor_assistance_startup_without_pedal_rotation = DEFAULT_MOTOR_ASSISTANCE_STARTUP_WITHOUT_PEDAL_ROTATION;
...
static void uart_receive_package(void)
{
  if (ui8_received_package_flag)
  {
    ...
    for (ui8_i = 0; ui8_i <= UART_NUMBER_DATA_BYTES_TO_RECEIVE + 1; ui8_i++)
    {
      ui16_crc_rx += ui8_rx_buffer[ui8_i];
    }

    // if CRC is correct read the package (8 bit value and therefore last one byte)
    if (ui8_rx_buffer[UART_NUMBER_DATA_BYTES_TO_RECEIVE + 2] == ui16_crc_rx)
    {
      ...
      // walk assist / cruise function 
      m_configuration_variables.ui8_walk_assist = DISPLAY_VLCD_WALK_ASSIST_ENABLED ? ((ui8_rx_buffer[1] & (1 << 5)) ? 1 : 0) : 0;

      // enable walk assist or cruise function depending on speed
      if (m_configuration_variables.ui8_walk_assist)
        m_configuration_variables.ui8_walk_assist = (ui16_wheel_speed_x10 < WALK_ASSIST_CRUISE_THRESHOLD_SPEED_X10) ? DEFAULT_WALK_ASSIST_FUNCTION_ENABLED : DEFAULT_CRUISE_FUNCTION_ENABLED;

      // assist level
      m_configuration_variables.ui8_assist_level_factor_x10 = m_configuration_variables.ui8_walk_assist ? ui8_walk_assist_levels_factor[ui8_assist_level] : ui8_assist_levels_factor_x10[ui8_assist_level];

      // startup motor power boost
      m_configuration_variables.ui8_startup_motor_power_boost_assist_level = ui8_startup_motor_power_boost_assist_levels[ui8_assist_level];

      // wheel perimeter
      m_configuration_variables.ui16_wheel_perimeter = DISPLAY_VLCD_WHEEL_PERIMETER_ENABLED ? ((uint16_t) ui8_rx_buffer[3] * 80) : DEFAULT_WHEEL_PERIMETER;

      // wheel max speed
      m_configuration_variables.ui8_wheel_max_speed = ui8_street_mode ? DEFAULT_STREET_MODE_SPEED_LIMIT : (DISPLAY_VLCD_WHEEL_MAX_SPEED_ENABLED ? ui8_rx_buffer[5] : DEFAULT_WHEEL_MAX_SPEED);
      ...
      // lights state
      m_configuration_variables.ui8_lights = DISPLAY_VLCD_LIGHTS_ENABLED ? (ui8_rx_buffer[1] & 1) : DEFAULT_LIGHTS_STATE;
        ...
        else
        {
          switch (ui8_assist_level)
          {
            ...
            case 1:
              ...
              // street mode
              ui8_street_mode = DEFAULT_STREET_MODE_FUNCTION_ENABLED ? ui8_lights_flag : 0;
            ...
            case 2:
              ...
              // startup motor boost enabled
              m_configuration_variables.ui8_startup_motor_power_boost_feature_enabled = ui8_lights_flag;

              break;

            case 3:

              // battery max power target enabled
              ui8_target_battery_max_power_enabled = ui8_lights_flag;
...
static void uart_send_package(void)
{
  ...
  // ADC torque sensor
  ui8_tx_buffer[3] = ui8_g_adc_torque_sensor_min_value;
  ...
  else if (ui8_system_state & ERROR_MOTOR_BLOCKED)
    ui8_tx_buffer[5] = DISPLAY_VLCD_FAULT_CODE_EBIKE_WHEEL_BLOCKED;
  else if (m_configuration_variables.ui8_temperature_current_limiting_value < 255)
  ...
  for (ui8_i = 0; ui8_i <= UART_NUMBER_DATA_BYTES_TO_SEND + 1; ui8_i++)
  {
    ui16_crc_tx += ui8_tx_buffer[ui8_i];
  }

  ui8_tx_buffer[UART_NUMBER_DATA_BYTES_TO_SEND + 2] = ui16_crc_tx;
